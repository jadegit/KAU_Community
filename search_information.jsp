...
<body>
	...
	<%
		...
		if(searchcategory.equals("id")) {
			rs = stmt.executeQuery("SELECT * FROM Informationdb WHERE ID Like '%" + searchword + "%';"); // SELECT 쿼리문 수행
		}
		else {
			rs = stmt.executeQuery("SELECT * FROM Informationdb WHERE Title Like '%" + searchword + "%';");
		}
	...
	<table border name="top menu">
		<tr>		
			<td ALIGN="center" WIDTH="250" Height="40" BGcolor="orange"
				onMouseover="MouseOver(this, 'darkorange');" onMouseout="MouseOut(this, 'orange');" onClick="go_information();">
	...
	<table name="catalog">
		...
		<%
		if(searchcategory.equals("id")) {
			rs = stmt.executeQuery("SELECT * FROM Informationdb WHERE ID Like '%" + searchword + "%' ORDER BY Num DESC;"); // SELECT 쿼리문 수행
		}
		else {
			rs = stmt.executeQuery("SELECT * FROM Informationdb WHERE Title Like '%" + searchword + "%' ORDER BY Num DESC;");
		}
