...
<title>Information forum</title>

<script language="JavaScript">
	function go_page(pagenum) {
		window.location = "information.jsp?page=" + pagenum;
	}
	function read_content(writingnum) {
		window.location = "readinformation.jsp?num=" + writingnum;
	}
</script>
...
<%
	...
	<table name="catalog">
		...
		for (int i=latest_wrt; latest_wrt-20<i; i=i-2) { // 한 페이지에 20개의 글
			if(i>0) {
				rs = stmt.executeQuery("SELECT * FROM Informationdb WHERE Num=" + i); // SELECT 쿼리문 수행
				rs.next();
		%>
		...
		<%	} 
			if(i-1>0) {
				rs = stmt.executeQuery("SELECT * FROM Informationdb WHERE Num=" + (i-1));
				rs.next();
		%>
		...
		<tr>
			<td colspan="2" ALIGN="center">
				<form action="search_information.jsp" method="get" name="searchform">
			...
			<td colspan="2"></td>
			<td ALIGN="right"><input type=button value="글쓰기" onClick="window.location='writinginformation.jsp';"></td>
		</tr>
...
<rignt>
<% 
rs = stmt.executeQuery("SELECT * FROM Informationdb ORDER BY View DESC;"); // 게시글을 역순으로 불러옴
%>
	<table>
		<tr>
			<td colspan="3"><strong>정보공유 베스트 글!!!</strong></td>
		</tr>
